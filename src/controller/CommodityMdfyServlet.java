...
public class CommodityMdfyServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(id!=null){//修改商品信息的请求
        	CommodityService service = new CommodityService();
        	Commodity cmdt = service.findCommodity(Integer.valueOf(id));
        	session.setAttribute("cmdt", cmdt);
            response.sendRedirect("modifyCommodity.jsp");
        }
        try {
            ...
            if (isMultipart) {
                ...
                int itemId = cmdt.getCommodityId();
            	...
            	cmdt = new Commodity(itemId, itemName, type, 0, Double.valueOf(price.toString()), uId, desc, imgsrc);
           	
            	if(service.updateCommodity(cmdt)){
            		session.removeAttribute("cmdt");
                    out.flush();//清空缓存
                    out.println("<script>");//输出script标签
                    out.println("alert('修改成功');");//js语句：输出alert语句
                    ...
                    out.flush();//清空缓存
                    out.println("<script>");//输出script标签
                    out.println("alert('修改后的商品非法');");//js语句：输出alert语句
                    out.println("window.location='UserInfo';");//js语句：输出网页回退语句
                    out.println("</script>");//输出script结尾标签
            	}
